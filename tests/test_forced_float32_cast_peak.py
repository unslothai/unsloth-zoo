...
def test_a_large_parameter_is_not_doubled():
    # Comfortably over the threshold so the staged path is taken.
    rows = int(_FORCED_FLOAT32_STAGE_BYTES // (2 * 1024)) + 1024
    big = torch.nn.Embedding(rows, 1024).cuda().to(torch.bfloat16)
