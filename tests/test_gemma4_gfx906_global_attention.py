...
def test_kernel_forward_backward_matches_sdpa(dtype, tol):
    ...
    def max_row_error_over_head_norm(a, b):
        # A row-local relative error is ill-conditioned for causal attention:
        # some gradient rows are analytically zero or arbitrarily small when the
        # scale=1 stress case saturates softmax. Normalize each row error by the
        # corresponding full-head reference norm instead. This still catches a
        # localized bad row without dividing harmless roundoff by ~0.
        row_diff = torch.linalg.vector_norm(a.float() - b.float(), dim=-1)
        head_norm = torch.linalg.vector_norm(
            b.float(), dim=(-2, -1)
        ).clamp_min(1e-6)
        return (row_diff / head_norm[..., None]).amax().item()
    ...
    for actual, expected in (
        ...
        assert max_row_error_over_head_norm(actual, expected) < tol * 4
...
@pytest.mark.skipif(not _runtime_is_gfx906(), reason="needs AMD gfx906")
def test_kernel_long_1025_matches_sdpa():
