...
def test_load_mlx_vlm_distributed_delegates_to_mlx_vlm_sharded_load(monkeypatch, tmp_path):
    ...
    (model_dir / "processor_config.json").write_text(json.dumps({"patch_size": None}))
    ...
    def sharded_load(repo, *, tensor_group=None, pipeline_group=None):
        ...
        patch_size = json.loads((Path(repo) / "processor_config.json").read_text()).get("patch_size")
        calls.append((repo, tensor_group, pipeline_group, patched_type, patch_size))
    ...
    tensor_group, pipeline_group = _FakeGroup(name="tensor"), _FakeGroup(name="pipeline")
    model, _processor = _load_mlx_vlm_distributed("fake/vlm", "qwen3_vl_moe", tensor_group=tensor_group, config_override_data={"model_type": "patched"}, sidecar_override_data={"processor_config.json": {"patch_size": 14}})
    for _ in range(3):
...
def test_from_pretrained_distributed_vlm_passes_override_without_temp_view(monkeypatch, tmp_path):
    ...
    model_path, calls, repair_paths = _write_config(tmp_path, config), [], []
    loaded_model = types.SimpleNamespace(_unsloth_mlx_config_view_paths=["patched-view"])
    monkeypatch.setattr(mlx_lm_utils, "_download", lambda *_a, **_k: model_path)
    monkeypatch.setattr(loader, "_materialize_mlx_vlm_sidecar_overrides", lambda *_a, **_k: (_ for _ in ()).throw(AssertionError("first temp view")))
    monkeypatch.setattr(loader, "_load_mlx_vlm_distributed", lambda target, *_a, config_override_data=None, sidecar_override_data=None, **_k: (calls.append((target, config_override_data, sidecar_override_data)), (loaded_model, types.SimpleNamespace(tokenizer=object())))[1])
    ...
    monkeypatch.setattr(loader, "_repair_degraded_vlm_processor", lambda processor, path, *_a, **_k: (repair_paths.append(path), processor)[1])
    ...
    assert calls == [("fake/vlm", {k: v for k, v in config.items() if k != "auto_map"} | {"model_type": "deepseekocr"}, None)]
    assert repair_paths == ["patched-view"]
    assert loaded_model._config_src_path == "patched-view"
