...
def test_buffering_child_is_not_called_stalled(hf_cache, monkeypatch):
    """A frozen partial while the child is still READING is hf_xet filling its reconstruction
    buffer, not a hang. Measured on a 20 Mbit/s throttled link: at 2.5 MB/s a 256MB per-file buffer
    is 100s of real network progress with nothing on disk, and the disk-only rule called that a
    stall and killed a perfectly healthy download."""
    blobs = _blobs_dir(hf_cache)
    (blobs / "buffering.incomplete").write_bytes(b"\0" * 1024)  # never grows

    counter = {"n": 0}

    def _io(_pid):
        counter["n"] += 1_000_000   # bytes keep arriving
        return counter["n"]

    monkeypatch.setattr(xf, "_child_io_bytes", _io)

    calls: list[str] = []
    stop = xf.start_watchdog(
        repo_ids = [REPO], on_stall = calls.append, interval = 0.05,
        stall_timeout = 0.2, buffering_timeout = 30.0, child_pid = 4242,
    )
    try:
        time.sleep(1.5)   # many stall_timeouts' worth
        assert not calls, f"watchdog killed a download that was still reading: {calls}"
    finally:
        stop.set()


def test_buffering_grace_is_bounded(hf_cache, monkeypatch):
    """Being busy is not the same as making progress: a child that reads forever without ever
    writing must still trip, just later. Otherwise a retry loop would never be detected."""
    blobs = _blobs_dir(hf_cache)
    (blobs / "busy.incomplete").write_bytes(b"\0" * 1024)
    ...
    stop = xf.start_watchdog(
        repo_ids = [REPO], on_stall = calls.append, interval = 0.05,
        stall_timeout = 0.2, buffering_timeout = 0.6, child_pid = 4242,
    )
    try:
        assert _wait(lambda: len(calls) >= 1, timeout = 5.0), \
            "the buffering grace never expired, so a busy-but-hung child is undetectable"
    finally:
        stop.set()
    assert "stalled" in calls[0].lower()


def test_stall_still_fires_when_the_child_reads_nothing(hf_cache, monkeypatch):
    """The ordinary hang: partial frozen AND no reads. The buffering clock must not weaken this."""
    blobs = _blobs_dir(hf_cache)
    (blobs / "hung.incomplete").write_bytes(b"\0" * 1024)

    monkeypatch.setattr(xf, "_child_io_bytes", lambda _pid: 12345)   # constant: no reads happening

    calls: list[str] = []
    stop = xf.start_watchdog(
        repo_ids = [REPO], on_stall = calls.append, interval = 0.05,
        stall_timeout = 0.2, buffering_timeout = 60.0, child_pid = 4242,
    )
    try:
        assert _wait(lambda: len(calls) >= 1, timeout = 3.0), \
            "a child doing no I/O at all was not detected as stalled"
    finally:
        stop.set()


def test_unavailable_io_counters_degrade_to_disk_only(hf_cache, monkeypatch):
    """macOS without psutil exposes no read counters. That must mean "no opinion" -- the old
    disk-only behaviour -- not "assume buffering forever"."""
    blobs = _blobs_dir(hf_cache)
    (blobs / "nocounters.incomplete").write_bytes(b"\0" * 1024)

    monkeypatch.setattr(xf, "_child_io_bytes", lambda _pid: None)
