...
The same heuristic exists in four drifted copies: three in ``dataset_utils``
here, and one in the trainer source ``unsloth/models/rl.py`` generates. Two
defects are shared by all of them, and together they produce
https://github.com/unslothai/unsloth/issues/2693:

1. They ask ``multiprocessing.get_start_method()`` whether forking is available,
   but ``Dataset.map`` imports ``Pool`` from ``multiprocess`` (the dill fork),
   which keeps its own independent default context. The guard and ``datasets``
   read different settings.

2. They use ``1`` as the "disable multiprocessing" sentinel. On ``datasets``
   >= 4.1 ``map()`` takes the pool branch for any ``num_proc >= 1``, so only
   ``None`` is in-process on every supported version.

This module lives here rather than in ``unsloth`` so that the trainer source
generated by ``unsloth/models/rl.py`` imports it without importing back into
``unsloth`` itself. Wiring the three ``dataset_utils`` copies onto it is a
follow-up: it changes behaviour for every consumer of this package, so it is
kept out of the change that introduces the policy.
"""
...
# away; a canary in tests/test_dataset_num_proc.py catches drift.
...
def get_dataset_num_proc(
    ...
    Args:
        ...
        serial_as_none: How to encode "run in-process". True at a ``map()`` call
            ...
            (``dataset_utils.sft_prepare_dataset``) treat a config
...
def _largest_split_rows(trainer) -> Optional[int]:
    ...
    skipped rather than abandoning the measurement: sizing happens per split, and
    an unsized one can never use workers whatever it is handed, so letting it
    hide a large sized sibling left that sibling on the uncapped auto
    ...
    for attribute in ("train_dataset", "eval_dataset"):
        ...
        for split in splits:
            ...
            try:
                ...
                # An unsized split cannot be parallelized either way, so skip it
...
def resolve_responses_only_num_proc(trainer, num_proc):
    ...
    ``dataset_utils.train_on_responses_only`` still auto-sizes with
    ...
    dill-pickled tokenizer closure -- the shape behind issue #2693. Until that
    copy is wired onto this module the bound is applied from outside, and two
    ...
    * **Serial is the config-layer sentinel, not the call-site one.** That helper
      ...
      ``__main__`` (#3211 / #3397), while ``None`` is safe because its auto path
      vetoes non-fork itself and lands in-process. That is exactly what
      ``get_dataset_num_proc(..., serial_as_none = False)`` encodes, so defer to
      it rather than mapping serial to ``1`` by hand.
    * **An explicit count disables its per-split small-split guard.** A
      ...
      a split is big enough to have been parallelized at all, which is
      what the row check below establishes.
    """
    # Mirror that helper's own test, so "explicit" means the same on both sides (it
    # treats bools as auto, since type(True) is not int).
    was_auto = num_proc is None or type(num_proc) is not int

    if not was_auto:
        # Explicit counts already bypass the small-split guard, so
    ...
    if rows is None or rows < ZOO_MIN_ROWS_FOR_MULTIPROC:
        ...
        if env_set and env_value is not None:
            return env_value
        # Otherwise it would have gone in-process anyway, and its guard
