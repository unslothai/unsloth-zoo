...
def chunked_hidden_states_selective_log_softmax(
    ...
    # Some VLM forwards can return a sequence length that differs from completion ids by a
    # small offset (for example, when additional multimodal tokens are present internally).
    # GRPO only needs completion-token alignment, so match both tensors on the trailing tokens.
    if hidden_states.shape[0] != index.shape[0]:
        raise RuntimeError(
            f"Unsloth GRPO expected matching batch size, got hidden_states={hidden_states.shape} and index={index.shape}"
        )
    if hidden_states.shape[1] != index.shape[1]:
        seq_len = min(hidden_states.shape[1], index.shape[1])
    ...
    all_per_token_logps = all_per_token_logps.reshape((index.shape[0], index.shape[1]))
    return all_per_token_logps


RL_REPLACEMENTS["grpo_selective_log_softmax"] = (
