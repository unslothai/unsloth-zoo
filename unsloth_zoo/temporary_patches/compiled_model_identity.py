...
"""Compiled classes (`__module__ == "unsloth_compiled_module_*"`) are still native transformers code.

transformers 5 skips checkpoint key conversions for `is_custom_code()` classes, so Inkling's
`model.llm.*` / `w13_weight` keys all came back UNEXPECTED and the model trained from random weights.
"""
