...
def patch_transformers_caching_allocator_warmup():
    ...
    warmup_fn = getattr(transformers.modeling_utils, "caching_allocator_warmup", None)
    if warmup_fn is None:
        return
    if getattr(warmup_fn, "__unsloth_allocator_warmup_guarded__", False):
        return

    def guarded_caching_allocator_warmup(*args, **kwargs):
