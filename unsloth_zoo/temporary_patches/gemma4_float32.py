    ...
    def forward(
        ...
        **kwargs,
        ...
        if attn_impl == "eager":
            ...
            # The FORCE_FLOAT32 Gemma-4 path intentionally bypasses
            # ALL_ATTENTION_FUNCTIONS for SDPA, so the gfx906 D=512 memory
            # fallback has to be offered here as well as in the registry wrapper.
            # It preserves the fp32 q/k/v policy; unsupported masks/shapes/dtypes
            # return None and take the exact existing SDPA path below.
            from .gemma4_gfx906_global_attention import maybe_gemma4_gfx906_global_attention
            attn_output = maybe_gemma4_gfx906_global_attention(
                self,
                query_states,
                key_states,
                value_states,
